...
- [x] **7.10** *(jul/2026, pedido explícito do usuário: diagrama de acorde no tap + redesign visual do Play Mode)* Não estava no roadmap original — item novo:
  - [x] **7.10a** Diagrama de acorde no tap — `@tombatossals/chords-db` (JSON puro, sem lib de renderização web-only) + `chord-diagram-lookup.ts` (parse do símbolo + lookup por classe de altura, testado contra o vocabulário real de `mapColonQualityToSuffix` do backend — os 9 valores batem 1:1 com suffixes do chords-db) + `ChordDiagram.tsx` (braço do violão 100% `react-native-svg`, sem lib de terceiros) + `ChordDiagramSheet.tsx` (bottom sheet, `@gorhom/bottom-sheet` — confirmado JÁ instalado, CLAUDE.md dizia "não instalado ainda", desatualizado). `ChordTokenLine` ganhou `onPressChord?` opcional — token vira `Pressable` só quando tem acorde E o consumidor passa o callback; usado tanto pelo Play Mode quanto pelo visualizador compartilhado (`ChordDiagramSheet` vive em `shared/` pelo mesmo motivo de `ChordTokenLine` ter sido promovido).
    - [x] **7.10a.1** *(jul/2026, pedido explícito: teclado além de violão, padrão CifraClub)* Teclado **não usa chords-db** (o pacote só tem `guitar.json`/`ukulele.json`, sem piano) — `lookupPianoChordShape()` computa as notas por teoria musical pura (intervalo a partir da raiz, mesmas 9 qualidades do backend), sem precisar de dataset de terceiro. `PianoChordDiagram.tsx` desenha 1 oitava (7 teclas brancas + 5 pretas) em `react-native-svg`, notas do acorde destacadas, raiz com cor distinta. 23 testes novos em `chord-diagram-lookup.test.ts` cobrindo os 9 intervalos + baixo de slash chord fora do tríade. **Correção (mesma rodada, feedback direto do usuário):** o seletor Violão/Teclado tinha ficado DENTRO do `ChordDiagramSheet` na primeira versão — usuário não queria escolher o instrumento a cada acorde tocado. Movido pra um controle único no topo da tela (`ChordSheetControlsSheet`, ver 7.10c) — escolhido uma vez, vale pra todos os acordes da música.
  - [x] **7.10b** Redesign de animação do Play Mode, só com o stack já aprovado (Reanimated 4 + `react-native-svg` + `expo-linear-gradient` + `expo-haptics` — nada de Skia/Moti/lib nova): linha ativa com transição em mola (`withSpring`) em vez de corte instantâneo de cor; barra de progresso da `PlayModeTopBar` com gradiente animado (`withTiming` suavizando o playhead); `SectionTransitionBadge` novo — pill efêmero que aparece ~1.7s ao entrar numa seção nova (verso/refrão), remonta via `key` a cada troca, sem estado externo; sublinhado pontilhado sutil nos acordes tocáveis (afordance permanente, não pulso único); micro-interações na `PlayModeBottomBar` (haptic + bounce no texto de velocidade, escala no botão de play ao pressionar). **Decisão confirmada com o usuário:** SEM animação de entrada escalonada nos pontos do diagrama de acorde (ponto a ponto) — geraria atraso perceptível pro músico que precisa ler rápido no palco. `ChordDiagram.tsx`/`PianoChordDiagram.tsx` continuam estáticos de propósito, não é uma lacuna.
  - [x] **7.10c** *(jul/2026, pedido explícito: toolbar única no topo — referência CifraClub — em vez de escolher instrumento a cada toque; design próprio, não clone visual)* `ChordSheetControlsSheet.tsx` — sheet único com 3 controles:
    - **Instrumento** — `InstrumentToggle.tsx` (Violão/Teclado), agora escolhido aqui, não mais dentro do `ChordDiagramSheet` (ver correção em 7.10a.1).
    - **Tom** — `TransposeStepper.tsx` (±semitom, -11..+11), desloca os acordes REAIS mostrados na cifra inteira (não só no diagrama).
    - **Capotraste** — `CapoPicker.tsx` (fileira de casas 1-12 + "Sem"), escondido quando instrumento = teclado (capotraste não existe pra teclado). Diferente de transpor: mantém a música no tom real, mas troca o que é MOSTRADO pela FORMA que se toca (capotraste na casa N = tocar uma forma N semitons abaixo do acorde real). Sheet do acorde mostra "`<acorde real>` com forma de `<forma>`" + badge da casa, mesmo padrão do CifraClub (referência visual do usuário).
    - `useChordSheetControls.ts` (shared/hooks/) centraliza o estado; `chord-transpose.ts` (`transposeChordSymbol`/`transposeTokenGrid`/`shouldPreferFlatsForKey`, espelhando `preferFlatsForKey` do backend) faz a transposição pura, 17 testes novos.
    - Botão de ajustes (ícone `SlidersHorizontal`) na `PlayModeTopBar` e no header do `SharedSongViewerScreen` abre o sheet.
    - `PlayModeTopBar` ganhou avatar do músico dono do repertório (`Avatar.tsx` já existente, anel gradiente teal — reaproveita `useMusician(repertoire.musician_id)`, sem endpoint novo). Não replicado no `SharedSongViewerScreen` (link público sem sessão — evitar depender de um endpoint que pode não ser acessível anonimamente sem verificar antes).
    - `PlayModeScreen.tsx` passou de 219 linhas (acima do limite de 200 do CLAUDE.md) — extraído `PlayModeChordSheetView.tsx` (loading/vazio/lista rolável) de volta pra 164 linhas.
  - ⚠️ **Não confirmado em device/emulador** (mesma limitação já registrada nos outros itens deste bloco) — `npx tsc --noEmit` limpo, mas as animações (especialmente a spring da linha ativa e o diagrama SVG do acorde) nunca foram vistas rodando de verdade.
...
### 7.11 — Cifra Pessoal e Comunidade (backend Bloco 8D)

- [~] **7.11a** — Camadas FSD completas em `features/musician`: tipos/validação Zod, adapters dos 15 endpoints de dono+comunidade, hooks TanStack Query e invalidação conjunta detalhe/view após editar
- [~] **7.11b** — Seis telas na `RepertoireStack`: lista pessoal, criação pela biblioteca, editor Visualizar/Editar, comunidade, detalhe read-only e importação com criação/análise da linha do importador
- [~] **7.11c** — Editor por overlay: corrigir/inserir/remover acorde, anotação pública ancorada, notas privadas separadas, ajustes persistentes, compartilhamento private/band/community e revisão de conflitos
- [~] **7.11d** — Entry points no header de Repertório e no Play Mode (somente dono); plano mobile alinhado a FREE=3 cifras e ESSENCIAL/PRO ilimitado+community
- [~] **7.11e** — Verificação automatizada: `npx tsc --noEmit`, `npx expo config --type public` e 100/100 testes Jest passam. Continua `[~]` até percorrer em device o fluxo completo contra o backend real (fork → edição → conflito → share → import) e validar ergonomia dos sheets/diagramas.
...
- [x] **10.5.1** — (14/jul/2026) `RoleSwitchSheet` (`navigation/components/`) com **@gorhom/bottom-sheet v5** instalado — JS puro (só depende de gesture-handler/reanimated, já no dev client → **não exige rebuild EAS**, mesma situação do react-native-draggable-flatlist); `BottomSheetModalProvider` adicionado no App.tsx. Contas com "Ativa agora"/check + switch instantâneo via `navigationRef`. Entradas: HomeAvatarMenu ("Trocar de conta", substituiu o "Ir para conta Fã" direto) e FanProfileScreen (linha acima do logout).
- [x] **10.5.2** — (14/jul/2026) CTA "Quero ser Músico também" no sheet (só quando falta a role) → mini-form CPF+celular inline (`BecomeMusicianForm`, Zod-only via `completeCadastroSchema` reaproveitado, mesma regra anti multi-conta do cadastro) → `POST /auth/add-role` → após o refresh o `useMusicianWizardGate` resolve `needs-wizard` e o RootNavigator troca a árvore pro wizard (1.13) sozinho — sem navigate manual.
- [x] **10.5.3** — (14/jul/2026) CTA "Quero ser Fã também" (um toque, sem dados extras) → `POST /auth/add-role` cria o aggregate audience com o mesmo sub → navega pra FanTabs (registrada no Root após o refresh).
- [x] **10.5.4** — (14/jul/2026) Backend: `POST /auth/add-role` (`AddRoleUseCase`, ver backend roadmap 4E.14 — assignRealmRole no Keycloak Admin API, compensação, reparo idempotente, 7 testes). App: `useAddRole` encadeia `refreshTokens()` no `onSuccess` (React Query aguarda a promise antes de resolver o `mutateAsync`) — roles novas entram no JWT e `buildAuthUser` deriva `musicianId`/`audienceId` do sub; transparente pro usuário.
...
| Testes automatizados (jul/2026, auditoria) | Unitário/lógica pura: **Jest + `jest-expo`** (preset oficial Expo SDK 56/New Architecture). E2E nativo, se/quando fizer sentido pós-MVP: **Maestro** (YAML, sem código nativo no projeto, flakiness <1%) — **não Detox** (setup mais pesado) e **não Cypress** (web-only, sem suporte a app nativo iOS/Android — issue oficial do próprio Cypress confirma isso em 2026; só reconsiderar se o app um dia rodar via `react-native-web`, o que não é o caso aqui). **Cypress fica reservado para o futuro `soundmeet-web`** (dashboard do estabelecimento, Fase 3, Next.js) — lá sim é a escolha padrão de mercado para E2E web |

---

## Ciclo de correções e evoluções — 14/jul/2026 (feedback de device do Wesley)

Implementado neste ciclo (detalhes nos commits):

- [x] Ícone do app: `icon-1024.png` (quadrado) + `adaptive-foreground-1024.png` (safe zone Android) gerados de `logo_verde-removebg-preview.png`; splash/notification icons atualizados no app.json. **Só aparece após novo build (EAS dev client)**.
- [x] Onboarding: swipe do carousel agora dá wrap circular nos dois sentidos (paridade com o autoplay).
- [x] Campo de senha: `defaultValue` (semi-uncontrolled) em campos `secureTextEntry` no FormField — round-trip do `value` controlado cancelava o preview nativo do último caractere no Android. Validar em device.
- [x] Placeholder de e-mail: `seuemail@email.com`.
- [x] Afinador redesenhado (referência Pinterest 654781233368977952): modo **Guitarra** (headstock SVG + chips E-A-D/G-B-E com corda ativa e check de afinada + pílula de cents deslizante) e modo **Cromático** (arco original). Detecção MPM idêntica nos dois; único gate de plano continua sendo o filtro de ruído (Essencial/Pro).
- [x] Home: avatar tappable → dropdown JS puro (`HomeAvatarMenu`) com Ver perfil / Ir para conta Fã (se role audience; FanTabs registrada como irmã no Root) / plano atual / Sair. Bottom sheet completo de roles continua sendo o 10.5.1.
- [x] Home: ícone **Mensagens** no header (chat saiu do tile "Agenda") — decisão: sem 6ª aba na tab bar; sem contador agregado (decisão anterior mantida).
- [x] ViewProfile: seção "Informações" (endereço/cidade, telefone, e-mail, plano, notas de preço).
- [x] Faixa de preço: **dois modelos simultâneos** (por hora E por evento) — backend `MusicianProfile.priceRanges` (colunas `price_hour_*`/`price_event_*`, migration com backfill), inputs com validação aninhada, filtros atualizados; form com blocos independentes por modelo.
- [x] Endereço + CEP do músico: `Location` VO estendido (street/number/complement/neighborhood/zip_code), autofill **ViaCEP** na seção Localização do EditProfile.
- [x] **Agenda real do músico** (`AgendaScreen` + `AvailabilityEditorScreen`): calendário mensal (shows = bookings confirmados via free-busy; bloqueios em coral), seleção de período no grid para férias/bloqueio, regras semanais por dia (ex.: sex ≥18h, sáb/dom o dia todo). Tile "Agenda" da Home agora abre a agenda (colisão de nomenclatura resolvida).
- [x] Busca por raio (backend 7.13a + app): chips 5/10/25/50 km no FanExplore com `expo-location` (leitura pontual foreground — **sem** tracking contínuo) e distância nos cards. **expo-location é módulo nativo → exige novo build EAS dev client** (agrupar com o rebuild do ícone).
- [x] Analytics: payload normalizado no adapter (defaults para campos ausentes — crash `top_requested_songs undefined` veio de imagem docker do backend defasada; imagem rebuildada).
- [x] Seeds: `soundmeet-backend/prisma/seed.ts` (`npm run seed -- --reset`) — 3 músicos (free/essential/pro, com endereço+CEP+coords, carteiras, faixas de preço), 4 estabelecimentos geolocalizados (SP×2, RJ, BH — testam o raio), 2 fãs, 4 eventos (todos os status), disponibilidade+férias, bookings (4 status), inquiry+conversa+mensagens, 10 pedidos (top songs), gorjetas (completed/pending/failed). Usuários Keycloak NÃO são criados pelo seed.

- [x] **Paywall de planos do músico** (`PlansPaywallScreen`, rota Root `Plans`, presentation modal): hero com coroa pulsante, toggle Mensal/Anual (−28% em destaque), 3 cards com entrada em stagger e seleção animada via `interpolateColor` (Essencial com moldura gradiente `premium` + badge "MAIS POPULAR"), stats de dinheiro por tier (taxa gorjeta/saque mínimo/prazo), CTA gradiente fixo que acompanha o tier selecionado. Valores espelhados do backend em `features/musician/domain/plans.config.ts`. Entrada: HomeAvatarMenu ("Ver planos e vantagens", substituiu o "em breve").
  - ⚠️ **Correção (06/ago/2026):** o texto original desta linha dizia que "o `plans-module` do backend **não tem controller HTTP**" e que por isso o CTA só informava que a cobrança estava em finalização. **Isso está desatualizado** — `src/nest-modules/plans-module/plans.controller.ts` existe com 7 rotas (`GET /plans` `@Public`; `GET/POST/DELETE /musicians/:id/subscription[/checkout]`; idem para `establishments`) e `AsaasSubscriptionGateway` real (`POST /v3/subscriptions`, `billingType: UNDEFINED` → o Asaas devolve um `invoiceUrl` hospedado em que o pagador escolhe PIX/cartão/boleto). Tarefa de ligação registrada no **Bloco 11.14**.
- [x] Gaps do ciclo corrigidos no backend: `envs/.env` com `ASAAS_API_KEY` (valor começa com `$`, expandia vazio) e `RESEND_API_KEY` (espaço após `=`, quebrava `source`) agora entre aspas simples — `source envs/.env` passa; specs pré-existentes fora de escopo alinhados aos tipos atuais (`create-establishment.use-case.spec.ts` ganhou `existing_establishment_ids`/`phone`, `payment.controller.spec.ts` ganhou `min_withdrawal_amount_brl`/`withdrawal_days` no mock) — `tsc --noEmit` limpo; `tip-fake.builder.ts` gera `Money` com `chance.floating({ fixed: 2 })` (default era 4 casas, estourava o VO).

- [x] **Geolocalização concluída por inteiro (7.13c)** — decisão: público = GPS pontual foreground (7.13a, já feito); músico/estabelecimento = endereço cadastrado geocodificado (CEP → coords, base fixa, sem tracking). Backend: busca de músicos por raio (colunas geo denormalizadas + migration com backfill + `searchByProximity` espelhado) e `IGeocodingService` (BrasilAPI CEP v2 → fallback Nominatim, best-effort) injetado nos updates de perfil de músico e estabelecimento. App: FanExplore com toggle **Locais | Músicos** (`ExploreModeToggle` + `MusicianResultCard` com distância/faixa de preço; busca por `stage_name`; chips de raio compartilhados via `RadiusChipsRow`; só a query da aba ativa roda). "Raio de atuação / cidades atendidas" registrado como 7.13d futuro no backend roadmap.
- [x] **Bloco 10.5 completo** (multi-role / account switching) — ver o bloco 10.5 acima; backend `POST /auth/add-role` (4E.14).
- [x] **Seed expandido pra TODOS os domínios** (mesmo padrão: aggregates + fake builders + repositórios Prisma): line-up dos eventos (EventMusician — destrava o fluxo fã evento→performers→pedido/gorjeta), presenças (EventAttendee), interações, gamificação completa (catálogo Badge, UserBadge com progresso/unlock, ledger UserScore + projeção UserPoints coerentes entre si), votos em pedido, transações (tips + saques completed/pending), biblioteca musical (6 músicas), repertórios dentro dos limites de plano (FREE 1 setlist / PRO), assinaturas de estabelecimento (Growth/Pro) e banda com membros. **3 bugs reais achados pelo seed e corrigidos:** (1) `RepertoirePrismaRepository.insert` quebrava com songs/invitees aninhados — o mapper repassava `repertoire_id` dentro do nested create e o Prisma rejeita ("Unknown argument"); o app nunca viu porque só exercitava o caminho do update/createMany; (2) `transaction-fake.builder` gerava Money com 4 casas decimais (mesmo bug do tip-fake.builder, corrigido no mesmo dia); (3) `transaction-fake.builder` engolia `null` explícito em `user_id`/`musician_id` (`?? new Uuid()`) e gerava UUID aleatório violando FK — saque não tem user_id.
